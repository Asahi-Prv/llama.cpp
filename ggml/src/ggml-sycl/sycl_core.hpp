...
class device_registry {
  ...
  private:
    device_registry() {
        ...
        // When ONEAPI_DEVICE_SELECTOR is not set the same physical GPU is
        // enumerated once per backend. Keep only the Level Zero copies: the
        // OpenCL ones would otherwise disable the Level Zero memory API and
        // hit driver issues. CPU devices are left untouched.
        std::vector<sycl::device> l0_devs;
        for (auto & d : gpu_devs) {
        ...
        if (!l0_devs.empty()) {
            m_devices = std::move(l0_devs);
        } else if (!gpu_devs.empty()) {
            // No Level Zero GPU: expose whatever is enumerated (CPU devices)
            // so non-SYCL tools keep working when SYCL is compiled.
